...
serve(async (req) => {
  ...
  try {
    ...
    const oddsUrl = `https://api.the-odds-api.com/v4/sports/${sport}/odds/?apiKey=${oddsApiKey}&markets=${markets}&oddsFormat=${oddsFormat}&dateFormat=${dateFormat}`
...
async function createPicksForGame(supabase: any, gameId: number, spread: any, total: number) {
  const picks = [
    { game_id: gameId, pick_type: 'spread', selection: 'home', line_value: spread.home },
    { game_id: gameId, pick_type: 'spread', selection: 'away', line_value: spread.away },
    { game_id: gameId, pick_type: 'total', selection: 'over', line_value: total },
    { game_id: gameId, pick_type: 'total', selection: 'under', line_value: total }
  ]
